...
DOC_STALE_PATTERNS := move_count_limit|bounded scalar assignment fallback|currently contains scaffolding|owner-aware, nearby, swap, sublist, reverse, k-opt, and ruin selectors .*not yet|no committed history yet|/home/pvd|/srv/lab/dev/solverforge|next release from this checkout|As of 2026-07-03|Install the published package and run|0\.17\.2|0\.18\.0.*not published|unpublished.*0\.18\.0|registry-backed builds remain intentionally blocked|release-base gate red|exact SolverForge.*registry dependencies are available|CVRP route callbacks|route callbacks for depot lookup|July 11, 2026|July 13, 2026|PyPI approval job|reviewed .*pypi.*environment|require manual approval|unreleased .*0\.6\.0|There is no .*v0\.6\.0 tag|PyPI latest .*0\.4\.0
