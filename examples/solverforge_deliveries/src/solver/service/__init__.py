...
from ...lib import DELIVERIES_SOLVER_CONFIG
from .payload import (
...
class DeliveryAppState:
    def __init__(self) -> None:
        self.manager = SolverManager(DELIVERIES_SOLVER_CONFIG)
        self._lock = threading.Lock()
        self._jobs: dict[str, JobRecord] = {}

    def create_job(self, plan: DeliveryPlan) -> JobRecord:
    ...
    def snapshot(self, record: JobRecord, revision: int | None = None) -> DeliveryPlan:
        plan = cast(DeliveryPlan, self.manager.snapshot(record.native_id, revision))
