...
Rust/Axum apps can mount `solverforge_ui::routes()`. Other hosts can read the
same embedded assets through `solverforge_ui::assets::get("sf.js")` and serve
the returned bytes, content type, and cache-control metadata from their own HTTP
framework.
...
| **Python/FastAPI** | Call a native binding over `solverforge_ui::assets::get()` and serve `/sf/{path}` with the returned metadata |
| **Tauri** | Add crate dep, serve via Tauri's asset protocol or custom command |
| **Any Rust HTTP server** | Call `solverforge_ui::assets::get(path)` and translate the returned `UiAsset` into that framework's response type |
...
and are validated before lookup:

```rust
if let Some(asset) = solverforge_ui::assets::get("sf.js") {
